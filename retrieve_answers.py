        ...
        for j in range(10):
            for k in range(10):
                for l in range(10):
                    ...
                    term, size_caviar = list[number]
                    filename = directory + "r" + str(i)+str(j)+str(k)+str(l)+".txt.txt"
                    with open(filename, "r", encoding="utf-8") as f:
                        contents = f.read()
...
def generate_plot(stats, caviar, kbe, total, yaxis_max = 5000):
    ...
    ax.set_title("for terms of fixed size 50, 1s", fontsize=14)
...
generate_plot(stats, cause_caviar, cause_kbe, total, 200)
